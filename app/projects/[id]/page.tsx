...
export default async function ServicePage({ params }: any) {
  ...
  const projects = await getProjectsByService(resolvedParams.service);
        ...
        <div className="grid md:grid-cols-4 gap-8">
          {projects.map((project: any) => (
            <Link
              ...
              key={project.id}
              className="group bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-200 hover:shadow-xl transition duration-300"
            >
              <div className="p-5">
